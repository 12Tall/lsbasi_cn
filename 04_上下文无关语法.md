...
1. 对于每一条规则 **R**：我们需要写一个同名的方法 **R()**，方法体与规则体结构、顺序都相同  
...
我们的语法中有两条规则：expr 和factor。我们先从factor 开始：根据`准则1`，我们需要创建一个`factor()` 方法；根据`准则4`，该方法只需要调用`eat(token)` 就好了。(*译注：有没有发现语法的起始符在最复杂的规则，而我们实现语法则是从最简单的规则入手*)  
...
很简单，对不对？  
继续！  
由`准则1`，规则expr 对应着`expr()`，方法体执行一次`factor()`；由`准则3`，紧跟着一个`while` 循环；由`准则2`，while 循环先是包含一个`if...else` 然后再执行依次`factor()` 在判断语句中同样也需要调用`eat(token)`校验token。将以上步骤组合在一起，我们就得到了`expr()`方法。  
...
参考下图，完成下面的问题：
![lsbasi-part4/lsbasi_part4_bnf1.png](img/04/lsbasi_part4_bnf1.png)  
